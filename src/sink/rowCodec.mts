...
import { tableConfigJSON } from '../definition.mjs';
...
export function convertCSV(content: string, lstFieldType: string[], doubleQuote: boolean = false): string {
...
export function csvToJsonArray(content: string, targetTable: string, lstFieldType: string[]): any[] {
    let retval: any[] = [];
    try {
        let lstLines = content.split(/\r\n/g);
...
export function jsonToCsv(filePath: string, tableConfig: tableConfigJSON, lstTableData: any[], emitBOM: boolean = false): Promise<void> {
