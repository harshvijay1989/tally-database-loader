...
import { connectionConfig, sinkAdapter } from '../definition.mjs';
...
export function loadDatabaseConfig(): connectionConfig {
    try {
...
export function applyCommandlineConfig(config: connectionConfig, lstConfigs: Map<string, string>): void {
    try {
        if (lstConfigs.has('database-technology')) config.technology = lstConfigs.get('database-technology') || '';
        if (lstConfigs.has('database-server')) config.server = lstConfigs.get('database-server') || '';
        if (lstConfigs.has('database-port')) config.port = parseInt(lstConfigs.get('database-port') || '0');
        if (lstConfigs.has('database-schema')) config.schema = lstConfigs.get('database-schema') || '';
        if (lstConfigs.has('database-username')) config.username = lstConfigs.get('database-username') || '';
        if (lstConfigs.has('database-password')) config.password = lstConfigs.get('database-password') || '';
        if (lstConfigs.has('database-loadmethod')) config.loadmethod = lstConfigs.get('database-loadmethod') || 'insert';
        if (lstConfigs.has('database-ssl')) config.ssl = lstConfigs.get('database-ssl') == 'true';

        config.technology = config.technology.toLowerCase(); //convert technology to lowercase

        //port = 0 [load default port for]
        if (config.port == 0) {
            if (config.technology == 'mssql') config.port = 1433;
            else if (config.technology == 'mysql') config.port = 3306;
            else if (config.technology == 'postgres') config.port = 5432;
            else;
        }
    } catch (err) {
...
let activeSink: sinkAdapter | undefined = undefined;

export function initSink(config: connectionConfig): sinkAdapter {
    activeSink = createSink(config);
    return activeSink;
}

export function getSink(): sinkAdapter {
    if (!activeSink) {
