...
async function upsertRecords(auth: SfAuth, objectApi: string, externalIdField: string, rows: Record<string, any>[]) {
    ...
    for (let i = 0; i < rows.length; i += 200) {
        ...
        for (const res of body) {
            ...
            else { result.failed++; if (result.errors.length < 10) result.errors.push((res.errors || []).map((e: any) => `${e.statusCode || ''} ${e.message}`).join('; ')); }
        }
    }
    return result;
}

async function runMapping(): Promise<any[]> {
    const auth = await getAuth();
    if (!auth) throw new Error('not connected to Salesforce');
    if (!fs.existsSync('./mapping.json')) throw new Error('no mapping saved yet');
    ...
    for (const om of (mapping.objectMappings || [])) {
        ...
        const mapped = rows.map(r => { const o: Record<string, any> = {}; for (const f of om.fields) o[f.target] = r[f.source]; return o; });
...
const server = http.createServer(async (req, res) => {
    ...
    try {
        ...
        if (url.pathname === '/api/run' && req.method === 'POST') {
            try { return json(200, { results: await runMapping() }); }
